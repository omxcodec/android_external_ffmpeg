...
ffmpeg: $(PRODUCT_OUT)/obj/STATIC_LIBRARIES/libvpx_intermediates/libvpx.a
	...
	$(REALTOP)/external/ffmpeg/configure \
		...
		--extra-ldflags="-nostdlib -Wl,-dynamic-linker,/system/bin/linker,-z,muldefs$(shell if test $(PRODUCT_SDK_VERSION) -lt 16; then echo -n ',-T$(REALTOP)/$(BUILD_SYSTEM)/armelf.x'; fi),-z,nocopyreloc,--no-undefined -L$(PRODUCT_OUT)/obj/STATIC_LIBRARIES -L$(PRODUCT_OUT)/system/lib -L$(PRODUCT_OUT)/obj/STATIC_LIBRARIES/libvpx_intermediates -ldl -lc" \
	...
	$(MAKE) install-libs DESTDIR=$(PRODUCT_OUT)
